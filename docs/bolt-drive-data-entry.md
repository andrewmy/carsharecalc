...
- `provider_id`, `vehicle_id`, `vehicle_name`, `vehicle_class`, `snowboard_fit`, `snowboard_source_url`, `fuel_type`, `consumption_l_per_100km_default`, `consumption_source_url`
...
- `fuel_type`: `petrol` / `diesel` / `ev` (hybrids treated as petrol; EVs don’t have self-fueling modeled yet).
- `consumption_l_per_100km_default`: fuel consumption estimate (blank for EV).
- `consumption_source_url`: link supporting the estimate.
...
4) Generate a fuel-consumption checklist (optional)

- `uv run python scripts/consumption_queue.py`

If your new Bolt row is missing `fuel_type` or consumption, paste the output into your PR description so it’s easy to backfill later.
...
- Dry-run (prints TSV snippets to paste):
  - `uv run python scripts/bolt_clone_tier.py --from-vehicle-id bolt_vw_tayron --to-vehicle-id bolt_vw_id4 --to-vehicle-name "VW ID.4" --snowboard-fit 2 --fuel-type ev`
- Apply directly:
  - `uv run python scripts/bolt_clone_tier.py --apply --from-vehicle-id bolt_vw_tayron --to-vehicle-id bolt_vw_id4 --to-vehicle-name "VW ID.4" --snowboard-fit 2 --fuel-type ev --as-of 2026-01-24`
